...
class LearningSwitch(api.Entity):
    ...
    def handle_link_down(self, port):
        ...
        """
        for link, pt in self.learned.iteritems():
            if pt == port:
                del self.learned[link]
    ...
    def handle_rx(self, packet, in_port):
        ...
        self.learned[packet.src] = in_port

        if packet.dst in self.learned:
            self.send(packet, self.learned[packet.dst], flood=False)
        else:
